...
rao2017 <- hic_exactTest(rao2017, parallel = FALSE) 
```
...
colnames(d) <- c("(Intercept)", "group", "factor")
knitr::kable(d)
```
...
rao_glm <- hic_glm(rao_glm, design = d, coef = 2, method = "QLFTest", p.method = "fdr", parallel = FALSE)
```
...
```{r}
# Print results as a data frame
knitr::kable(head(results(rao_glm)))
```
